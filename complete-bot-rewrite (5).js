...
module.exports = (client) => {
  ...
  /**
   * Tính tổng thời gian macro từ webhook messages trong channel
   * Loại bỏ khoảng nghỉ (break) - chỉ tính thời gian macro liên tục
   * 
   * @param {Array} times - Mảng timestamps của webhook messages
   * @param {Number} maxGap - Khoảng cách tối đa giữa 2 message (ms) để vẫn tính là cùng session
   * @returns {Number} Tổng thời gian macro (ms)
   */
  function calcActive(times, maxGap = 10 * 60 * 1000) {
    if (!times || times.length === 0) return 0;
    if (times.length === 1) return 0; // 1 message = 0 thời gian
    ...
    // Duyệt qua từng cặp message liên tiếp
    for (let i = 1; i < sorted.length; i++) {
      const gap = sorted[i] - sorted[i - 1];
      
      // Nếu gap <= maxGap → cùng session, cộng vào tổng
      // Nếu gap > maxGap → user nghỉ, bỏ qua gap này
      if (gap <= maxGap) {
        totalActive += gap;
      }
    }
  ...
  /**
   * Fetch và tính thời gian macro từ webhook messages trong channel
   * Dùng cho daily check để tính chính xác thời gian hoạt động
   * 
   * @param {Channel} channel - Discord channel
   * @param {Number} limit - Số message tối đa fetch (MAX 100 do Discord API limit)
   * @returns {Number} Tổng thời gian macro (ms)
   */
  async function fetchAndCalcActiveTime(channel, limit = 100) {
    try {
      // ⚠️ Discord API LIMIT: Max 100 messages per request
      const actualLimit = Math.min(limit, 100);
      
      // Fetch messages từ channel
      const messages = await channel.messages.fetch({ limit: actualLimit });
      
      // Lọc chỉ lấy webhook messages
      const webhookMessages = messages.filter(m => m.webhookId);
      
      if (webhookMessages.size === 0) {
        console.log('⚠️ No webhook messages in', channel.name);
        return 0;
      }
      ...
      console.log(`📊 ${channel.name}: ${webhookMessages.size} webhook messages found`);
      
      // Tính thời gian
      return calcActive(times);
      
    } catch (err) {
      console.error('❌ Fetch error for', channel.name, ':', err.message);
      return 0;
    }
  }
  ...
  client.on('messageCreate', async (msg) => {
    try {
      ...
      // ⚠️ CHỈ RESET KHI THỰC SỰ LÀ NGÀY MỚI (không phải lần đầu load)
      if (d.date && d.date !== today) {
        d.times = [];
        d.date = today;
        console.log('  📅 New day detected, reset times');
      } else if (!d.date) {
        // Lần đầu load data - set date nhưng GIỮ NGUYÊN times
        d.date = today;
        console.log('  📅 First load, keeping existing times');
      }
      ...
      if (ch.parentId === CAT.SLEEP || ch.parentId === CAT.EMPTY) {
        ...
        if (biomeKey) {
          ...
          if (biomeKeys.length > 0) {
            ...
            // XỬ LÝ BIOME ĐẦU TIÊN
            await handleBiome(ch, biomeKeys[0]);
            
            // ⚠️ CHỈ THÊM BADGES CÒN LẠI, KHÔNG DI CHUYỂN CHANNEL NỮA!
            for (let i = 1; i < biomeKeys.length; i++) {
              ...
              if (idx !== -1) {
                ...
                // Add new badge (KHÔNG DI CHUYỂN CHANNEL)
                d.badges.push(biome.badge);
                console.log('➕ Badge added (additional):', biome.badge);
              }
            }
            
            // Rename sau khi thêm hết badges
            await safeRename(ch, d.streak, d.badges);
    ...
    try {
      ...
      for (const [, ch] of channels) {
        try {
          ...
          // 🔥 FETCH VÀ TÍNH THỜI GIAN TỪ WEBHOOK MESSAGES (MAX 100)
          console.log(`\n🔍 Processing: ${ch.name}`);
          const active = await fetchAndCalcActiveTime(ch, 100);
